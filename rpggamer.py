...
stock = {'shield':(15,20,50),
         ...
         'pole':(10,5,50),
         'rope':(10,5,70)}
armour_types = set(['shield','cuirass','gauntlet'])
...
while len(players) < max_players:
    ...
    life = (profile['Muscle'] + (profile['Speed']) + random.randint(5,25))
    magic = (profile['Brainz'] + (profile['Charm']) + random.randint(5,25))
    prot = (profile['Speed'] + (profile['Brainz']) + random.randint(5,25))
    gold = random.randint(9,49) + random.randint(9,49) + random.randint(5,25)
    # Validate stats
    if life > 0 and life < 200:
        profile['life'] = life
    else:
        life = random.randint(9,99)
    if magic > 0 and magic < 200:
        profile['magic'] = magic
    else:
        magic = random.randint(9,99)
    if prot > 0 and prot < 200:
        profile['prot'] = prot
    else:
